...
const UseCallbackExam = () => {
  ...
  return (
    <div className="m-3">
      ...
      <button className="border border-orange-500 p-2 m-2" onClick={() => setBoxSize(boxSize + 10)}>
        크기 증가
      </button>
      <button className="border border-orange-500 p-2 m-2" onClick={() => setBoxColor((boxColor + 1) % 3)}>
        컬러 변경
      </button>
      <button className="border border-orange-500 p-2 m-2" onClick={() => setNumber(number + 1)}>
        숫자 변경
      </button>
      {/* 자식 컴포넌트에 함수를 props로 넘기기 */}
