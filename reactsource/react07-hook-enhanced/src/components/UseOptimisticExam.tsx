...
import { initMessages, type Message, type ThreadProps } from "../types";
...
const Thread = ({ messages, sendMessage }: ThreadProps) => {
  ...
  // 낙관적 상태 업데이트
  const [optimisticMessage, addOptimisticMessage] = useOptimistic<Message[], string>(messages, (state, newMessage) => [
    ...state,
  ...
  return (
    <div>
      {/* 메세지 보여주기 */}
      {optimisticMessage.map((message, idx) => (
        <div key={idx}>
          {message.text}
      ...
      <form action={formAction} ref={formRef}>
        <input type="text" name="message" placeholder="메세지를 입력해 주세요" className="border p-2" />
...
// 메세지를 서버로 전송하는 것을 표현한 함수 정의
// 실제 네트워크 요청처럼 보이도록 1초후 전달받은 메시지 반환
